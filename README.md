# Purple Blossom Theme 🌸

A dreamy dark theme for Visual Studio Code featuring soft purple hues and delicate pale pink accents.
<img width="9917" height="7001" alt="wall" src="https://github.com/user-attachments/assets/3420715a-d748-4e79-be35-70d74ca71bfe" />
## Features

 **Elegant Color Palette** - Carefully balanced purple and pink tones  
 **Easy on the Eyes** - Low-contrast design perfect for long coding sessions  
 **Dark Mode Optimized** - Ideal for late-night coding  
 **Excellent Readability** - Clear syntax highlighting with distinct colors  
 **Cohesive Design** - Unified color scheme across all UI elements  

## Installation

### Manual Installation 

1. Clone this repository
2. Copy the theme folder to your VS Code extensions directory:
   - **Windows**: `%USERPROFILE%\.vscode\extensions`
   - **Mac/Linux**: `~/.vscode/extensions`
3. Reload VS Code
4. Select the theme from `File > Preferences > Color Theme`
5. Or you can download latest release unpack the repo into extention folder.

## Color Palette

| Color | Hex | Usage |
|-------|-----|-------|
| Deep Purple | `#1a1625` | Editor background |
| Pale Pink | `#ffb3e6` | Strings |
| Lavender | `#e0b3ff` | Constants, types |
| Medium Purple | `#c084fc` | Keywords |
| Soft Pink | `#f0c4f0` | Variables |
| Plum | `#dda0dd` | Functions |

## Screenshots

<img width="2495" height="1440" alt="preview (1)" src="https://github.com/user-attachments/assets/85ea9874-b17a-49e5-8acf-fa5b9318efa2" />

<img width="1972" height="1093" alt="preview (2)" src="https://github.com/user-attachments/assets/0e18e4e7-a87e-40ee-b29d-22f456dda621" />


## Feedback & Contributions

Found a bug or have a suggestion? Please email me yanishmoshidora@gmail.com

Pull requests are welcome! Feel free to contribute improvements.

## License

MIT License - see (LICENSE) file for details.

## Acknowledgments

Created with 💜 for developers who love purple and pink aesthetics.

---

**Enjoy coding with Purple Blossom!** 
